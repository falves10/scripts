...
namespace HZG {
    ...
    void TruthSelect::dumptruth(){
                ...
                {       
                        ...
                        //cout<<iparticle<<endl;
			             if(fabs(ptcl->pdgId()) <= 25 )
                         {
                             if(ptcl->nParents()!=0)
                             {
                                cout<<"pid="<<ptcl->pdgId()<<" pt="<<ptcl->pt()<<" eta="<<ptcl->eta()<<" phi="<<ptcl->phi()<<" barcode="<<ptcl->barcode()<<" status="<<ptcl->status()<<" m="<<ptcl->m()<<" nChild="<<ptcl->nChildren()<<" e="<<ptcl->e()<<" p="<<ptcl->p4().P()<<" et="<<ptcl->p4().Et()<<" e/p="<<ptcl->e()/ptcl->p4().P()<<" et/pt="<<ptcl->p4().Et()/ptcl->pt() << " parent pdgID " << ptcl->parent(0)->pdgId() << " parent barcode " << ptcl->parent(0)->barcode() << endl ;
                             }
                            
                             
                             
                            //cout<<"pid="<<ptcl->pdgId()<<" pt="<<ptcl->pt()<<" eta="<<ptcl->eta()<<" phi="<<ptcl->phi()<<" barcode="<<ptcl->barcode()<<" status="<<ptcl->status()<<" parent pdgID " << ptcl->parent(0)->pdgId() << " parent barcode " << ptcl->parent(0)->barcode() << endl ;  
                         }
    ...
    vector<const xAOD::TruthParticle*> TruthSelect::TruthLeptonsFromZ()
    {
        ...
        {      
                        ...
                        //    if (ptcl->pdgId()==25&&ptcl->status()>60) HG::printTruthPtcl(ptcl,"The higgs");
                        //if(ptcl->status() != 1 && ptcl->barcode() > 20000) continue; //check if the particle is stable
                        //if(ptcl->status() != 1) continue;
                        //if(fabs(ptcl->pdgId())>100) continue;
                        //if(! notFromHadronic(ptcl)) continue; 
                        ...
                        if(isStable)
                        {
                            if (abs(ptcl->pdgId())== 11 && ptcl->nParents()!=0) 
                            {
                                ...
                                {
                                    //cout<<" PROMPT father ID "<< ptcl->parent(0)->pdgId() << " father barcode " << ptcl->parent(0)->barcode() << endl;
                                ...
                                {
                                    //cout<<"id particle "<< ptcl->pdgId() << " paricle barcode " << ptcl->barcode() << endl;
                                    int ancestorID = AncestorID(ptcl);
                                    //cout<<" NON-PROMPT electrons (fatherID): "<< ptcl->parent(0)->pdgId() << " Ancestor ID " << ancestorID << endl;

                                    if( ancestorID == 23 )
                                    {
                                        //cout<< " FSR chain, father is Z " << ancestorID << endl;
                                        tmp_vector.push_back(ptcl);
    ...
    vector<const xAOD::TruthParticle*> TruthSelect::GetLargestpTLeptons(vector<const xAOD::TruthParticle*> vec)
    {
        ...
        //cout<<" leading function " << pos_l1 << endl;
        //cout<<" subleading function " << pos_l2 << endl;
	...
	void TruthSelect::Reco_Truth_Matching(Object_llg &_llg)
	{
		...
		//cout<<" _llg.m_channel " << _llg.m_channel << endl;
		if(_llg.m_channel==1){
			
			//cout<< " selected electrons size " << (*_llg.m_selected_electrons).size() << endl;
			for (auto elec : (*_llg.m_selected_electrons) ) 
			{
				//cout<< " elec pts "<< elec->pt()/1000 << endl;  
			}
			//cout<< "pt elec 2 "<< (*_llg.m_selected_electrons)[_llg.index_lepton2].pt() << endl;
			...
			//cout<< " selected muons  size " << (*_llg.m_selected_muons).size() << endl;
			for (auto mu : (*_llg.m_selected_muons) )  
                        {
                                //cout<< " muons pts "<< mu->pt()/1000 << endl;
                        }
			//cout<< "pt muon 1 "<< (*_llg.m_selected_muons)[_llg.index_lepton1].pt() << endl;
                        //cout<< "pt muon 2 "<< (*_llg.m_selected_muons)[_llg.index_lepton2].pt() << endl;
			(*m_beventmap)["l1_istruth"] = isTruthMuon((*_llg.m_selected_muons)[_llg.index_lepton1]);
			//cout<<" l1_istruth : "<< (*m_beventmap)["l1_istruth"] << endl;
			(*m_beventmap)["l2_istruth"] = isTruthMuon((*_llg.m_selected_muons)[_llg.index_lepton2]);
			//cout<<" l2_istruth : "<< (*m_beventmap)["l2_istruth"] << endl;

			/*
			const xAOD::Muon *lep1=(*_llg.m_selected_muons)[_llg.index_lepton1];
			const xAOD::TruthParticle* truth_lep1 = xAOD::TruthHelpers::getTruthParticle(*lep1);

			const xAOD::Muon *lep2=(*_llg.m_selected_muons)[_llg.index_lepton2];
                        const xAOD::TruthParticle* truth_lep2 = xAOD::TruthHelpers::getTruthParticle(*lep2);

			if(!truth_lep1 || !truth_lep2 ) { cout<< " broken link " << endl;}
			else cout<<" lep1 truth particle " << truth_lep1->pdgId() << endl; 
			//cout<<" works " << endl;//cout<<" lep1 truth particle " << truth_lep1->pdgId() << " lep2 truth particle " << truth_lep2->pdgId() << endl;
			*/

			/*
			const xAOD::TruthParticle *father_lep1;
			const xAOD::TruthParticle *father_lep2;

			const xAOD::TruthParticle *grandfather_lep1;
			const xAOD::TruthParticle *grandfather_lep2;
			...
			if(truth_lep1->nParents()!=0 ) 
			{ 
				father_lep1 = truth_lep1->parent(0); ID_father_1 = father_lep1->pdgId(); 
				grandfather_lep1 = father_lep1->parent(0); ID_gfather_1 = grandfather_lep1->pdgId();
			}
			if(truth_lep2->nParents()!=0 ) 
			{ 
				father_lep2 = truth_lep2->parent(0); ID_father_2 = father_lep2->pdgId(); 
				grandfather_lep2 = father_lep1->parent(0); ID_gfather_2 = grandfather_lep2->pdgId();
			}
			

			cout<<" father lep1 ID " << ID_father_1  << " father lep2 " << ID_father_2 << " grandfather lep1 "  << ID_gfather_1 << " grandfather lep2 " << ID_gfather_1 << endl; 
			*/
		}
	}
	...
	int TruthSelect::AncestorID(const xAOD::TruthParticle *ptcl){
	  ...
	  const xAOD::TruthParticle *grandfather;
	  if (ptcl->nParents()!=0) { father = ptcl->parent(0); ID = father->pdgId();} else return ID;
	  //cout<<" father ID " << ID << endl;
	  if (father->nParents()!=0) { grandfather = father->parent(0); ID = grandfather->pdgId();} else return ID;
	  //cout<<" grandfather ID " << ID << endl;
	  while (grandfather->nParents()!=0){
	    father = grandfather->parent(0);
	    grandfather = father ;
	    //cout<< " ID while loop " << ID << endl;
